...
class ObjectViewed(models.Model):
    ...
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE) # User model or Product model or Order model or Cart model or Address model ...
    object_id = models.PositiveIntegerField() # User id or Product id or Order id ...
    content_object = GenericForeignKey('content_type', 'object_id') # Product instance or User instance or Order instance ...
...
def object_viewed_receiver(sender, instance, request, *args, **kwarg):
    c_type = ContentType.objects.get_for_model(sender) # sender is same as instance.__class__
    user = None
...
def post_save_session_receiver(sender, instance, created, request, *args, **kwarg):
    if created:
...
def post_save_user_changed_receiver(sender, instance, created, request, *args, **kwarg):
    if not created:
...
def user_logged_in_receiver(sender, instance, request, *args, **kwarg):
    user = instance
    ip_address = get_client_ip(request)
